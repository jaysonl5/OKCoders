...
function processPresidentDataRow(rowCells) {
	...
	var partyData = getPresidentPartyFromRow(rowCells)
	// add an additional piece of row data here
	// example
	// var firstTermParty = getPartyFromRow(rowCells)
	presidents.push({ name: nameData, party: partyData })
}
...
function getPresidentPartyFromRow(rowCells) {
	let party = ""
	rowCells.forEach((element, index) => {
		var cellWithPresidentDataPosition = 9
